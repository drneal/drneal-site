...
read_time: 25 min
summary: "Before Chapter 4 throws you into the deep end, build a working neural network from scratch â in a spreadsheet. No code. No libraries. Just arithmetic you can see and touch."
featured: true
---
...
*This article accompanies Jeremy Howard's fast.ai Practical Deep Learning for Coders 2022 Â· Lesson 3, specifically the section from 1:04:30 to 1:20:47. It is part of the curriculum materials produced by Dr Neal for one-to-one AI Learners sessions.*
...
[â¶ Watch Lesson 3 on YouTube â jump to 1:04:30](https://www.youtube.com/watch?v=hBBOjCiFcuo&t=3870s)

---

## A Word Before You Start: This Is Chapter 4's Prelude

Chapter 4 of *Deep Learning for Coders with fastai and PyTorch* is where the hairy stuff really begins. It covers MNIST from scratch â building a complete image classifier using nothing but raw PyTorch tensors, gradient descent, and the same mathematical machinery that runs every serious deep learning model in the world. It is not light reading.

The exercise in this post is Jeremy Howard's recommended prelude to that chapter. If you can build a working neural network in a spreadsheet â and actually understand every cell â you will find Chapter 4 considerably less intimidating. The algebra is identical. The only difference is that the spreadsheet makes every intermediate value visible and touchable, whereas Python compresses it into a few lines of tensor arithmetic. If you have not yet worked through the full Lesson 3 material â gradient descent, ReLU, and how neural networks approximate any function â [read that companion post first](/post/2026-06-19-how-deep-neural-networks-really-work) before attempting the steps below.
...
<div class="callout">
ð¡ <strong>Three versions of this exercise, in increasing order of ambition:</strong><br/>
1. <strong>Excel</strong> (Windows/Mac) â follow the steps below exactly as Jeremy does in the video<br/>
2. <strong>OpenOffice / LibreOffice Calc</strong> (free, open source, any OS) â identical logic, equivalent commands<br/>
3. <strong>Python / NumPy</strong> (ambitious) â if you want to translate every step to code before tackling Chapter 4
</div>
...
ð¥ <strong>Download links (both are free and open source):</strong><br/><br/>
â <a href="https://www.libreoffice.org/download/download-libreoffice/" target="_blank">LibreOffice (recommended â most actively maintained)</a><br/>
â <a href="https://www.openoffice.org/download/" target="_blank">Apache OpenOffice</a><br/><br/>
Both are completely free, contain no ads, and require no account or licence key. LibreOffice is the version most developers recommend today. For the purposes of this exercise, either works identically â all the commands below apply to both.
</div>
...
**Stage 1 â Linear Regression Model:** Multiply each passenger feature by a learnable coefficient, sum the results, and produce a single prediction number. This is a single neuron.

**Stage 2 â Neural Network:** Take two of those linear neurons, apply a ReLU (replace negatives with zero) to each, and add them together. This is a one-hidden-layer neural network â the same fundamental structure as modern deep learning, just tiny.
...
### Step 1 â Download the Titanic dataset
...
[â¶ Download titanic.xlsx from GitHub](https://github.com/fastai/course22/tree/master/xl)

If using your own CSV, open it in Excel or LibreOffice Calc via **File â Open**.

---

### Step 2 â Prepare the feature columns
...
| Column | Label | Notes |
|---|---|---|
| A | `survived` | 0 = died, 1 = survived (this is your **target** â what you're predicting) |
...
**OpenOffice/LibreOffice Calc â identical formula:**
...
### Step 3 â Create a parameters row

Above your data (say, in Row 1 if your header is in Row 2, or use a clearly labelled section at the top of the sheet), create **one cell per feature** (7 cells for 7 features). These are your model's learnable coefficients â what gradient descent will adjust.
...
<div class="callout">
â ï¸ <strong>Important:</strong> Once you have entered the parameters, press <strong>F9</strong> (recalculate) once to lock in the random starting values, then <em>immediately copy the cells and paste as Values Only</em> (Paste Special â Values). This prevents the parameters from re-randomising every time the sheet recalculates. You want the Solver to change the values, not RAND() to keep resetting them.
</div>
...
### Step 4 â Compute predictions for each passenger
...
**Excel â in the Prediction column (e.g. column I), enter for row 2:**
<div class="excel-cmd">
...
**OpenOffice/LibreOffice Calc â identical:**
<div class="oo-cmd">
=SUMPRODUCT($B$1:$H$1, B2:H2)
</div>
...
**Excel â in a dedicated cell (e.g. K1):**
<div class="excel-cmd">
=AVERAGE(ABS(I2:I892 - A2:A892))
</div>

**Important:** This is an **array formula**. In Excel, enter it with **Ctrl+Shift+Enter** (not just Enter), and you will see curly braces `{ }` appear around it automatically. In Excel 365 / Excel 2019+, you can press Enter normally â array formulas are automatic.
...
Label this cell clearly â e.g. put `Loss (MAE)` in the cell to the left. This is the number the Solver will minimise.

---

### Step 6 â Run Solver to train the model

This is gradient descent â the same optimisation that trains GPT, image classifiers, and every other neural network. The Solver finds parameter values that minimise your loss.

**In Microsoft Excel:**

1. Go to **Data** tab â **Solver** (if you don't see Solver: File â Options â Add-ins â Manage Excel Add-ins â tick Solver Add-in â OK)
...
A single linear model has a fundamental limitation â it can only draw a straight line (or flat hyperplane) through the data. Adding a non-linearity (ReLU) and a second linear unit gives the network the ability to learn curved, complex boundaries.

### Step 7 â Add a second set of parameters
...
### Step 8 â Compute two predictions and apply ReLU

For each passenger, compute two raw predictions â one from each parameter row â then apply a **Rectified Linear Unit (ReLU)**: replace any negative value with zero. This is the non-linearity that gives neural networks their power.

**Excel â Raw prediction from params_1 (column J):**
<div class="excel-cmd">
=SUMPRODUCT($B$1:$H$1, B3:H3)
</div>

**Excel â ReLU of first prediction (column K):**
<div class="excel-cmd">
=MAX(J3, 0)
</div>

**Excel â Raw prediction from params_2 (column L):**
<div class="excel-cmd">
=SUMPRODUCT($B$2:$H$2, B3:H3)
</div>

**Excel â ReLU of second prediction (column M):**
<div class="excel-cmd">
=MAX(L3, 0)
</div>

**Excel â Final neural net output = sum of the two ReLU outputs (column N):**
<div class="excel-cmd">
=K3 + M3
</div>

**OpenOffice/LibreOffice Calc â all identical:**
<div class="oo-cmd">
=SUMPRODUCT($B$1:$H$1, B3:H3)    â first raw pred
=MAX(J3, 0)                        â ReLU
=SUMPRODUCT($B$2:$H$2, B3:H3)    â second raw pred
=MAX(L3, 0)                        â ReLU
=K3 + M3                           â neural net output
</div>
...
### Step 10 â Train the neural network with Solver

Run Solver again, this time with **both** parameter rows as the variable cells.

**Excel:**
1. Data â Solver
2. Set Objective: loss cell â Min
3. By Changing Variable Cells: select **both** parameter rows together, e.g. `$B$1:$H$2`
4. Method: GRG Nonlinear â Solve

**LibreOffice Calc:**
1. Tools â Solver
2. Target cell: loss â Minimum
3. By changing cells: both parameter rows

The loss should now be lower than before â the two-neuron network is more expressive than the single linear model.
...
### Step 11 â Compute all predictions at once with MMULT
...
This produces a matrix where each row is a passenger and each column is one neuron's raw output â the equivalent of all your SUMPRODUCT formulas at once.
...
In both applications, enter this as an array formula (**Ctrl+Shift+Enter**) and select a destination range the right size (number of passengers Ã number of neurons) before entering.

<div class="callout">
ð¡ This is exactly what PyTorch's <code>@</code> operator or <code>torch.matmul()</code> does â and it is the reason GPUs are so effective at deep learning. They can perform enormous matrix multiplications (thousands of rows, thousands of columns) in parallel.
</div>
...
Every concept in Chapter 4 of the book â stochastic gradient descent, the forward pass, the activation function, the loss â maps directly onto what you have done in this spreadsheet. The book just compresses it into clean Python, vectorised across thousands of images simultaneously.
...
From here you would implement gradient descent manually â a fine exercise that will make Chapter 4's PyTorch autograd feel like a revelation rather than a mystery.

---

## Why This Exercise Is Worth Your Time

The fast.ai philosophy is top-down: you use the tools before you understand the internals. But at the Chapter 4 inflection point, the book inverts â it asks you to understand the internals before you can properly use the tools. The spreadsheet exercise bridges that inversion. It makes the internals concrete and tactile at the cost of only a couple of hours.

Jeremy has said this exercise is one of his strongest recommendations to students who find Chapter 4 difficult. Having built a neural network with your own hands â even a tiny one, even in a spreadsheet â permanently changes the way you read the Python code that follows.

Do it in Excel. Do it in LibreOffice. Or do it in Python. But do it before you open Chapter 4. And if you haven't yet worked through the Lesson 3 foundations, [start with the companion guide â](/post/2026-06-19-how-deep-neural-networks-really-work)

---

<div class="notebooklm-banner">
ð <strong>NotebookLM companion resource:</strong><br/>
...
<a href="https://notebooklm.google.com/notebook/8e7184c3-a0c5-47a5-8992-85e90f8b5261" target="_blank">â Open NotebookLM Notebook (Google account required)</a>
</div>
...
Students who study with Dr. Neal complete the fast.ai curriculum in half the median time, with substantially deeper practical understanding â and they leave with a project, not just a certificate.

[â Contact Dr. Neal Aggarwal for 1-to-1 sessions, group workshops, and curriculum design](/about#contact)
